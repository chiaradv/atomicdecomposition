...
public class Display extends JComponent {
     ...
     *            {@link prefuse.data.expression.parser.ExpressionParser} to get
     *            a {@link prefuse.data.expression.Predicate} instance. */
    ...
    protected void registerDefaultCommands() {
        ...
        try {
            registerKeyboardAction(new ExportDisplayAction(this), "export display",
                    KeyStroke.getKeyStroke("ctrl E"), WHEN_FOCUSED);
     ...
     *         {@link prefuse.visual.VisualItem#getBounds()} are not accurate
     *         and the item's {@link prefuse.render.Renderer} is drawing outside
     *         of the reported bounds. In this case, there is usually a bug in
     *         the Renderer. One reported problem arises from Java itself,
     *         however, which inaccurately redraws images outside of their
     *         reported bounds. If you have a visulization with a number of
     *         images and are seeing rendering artifacts, try disabling
     *         damage/redraw. */
     ...
     *            repaint) {@link prefuse.visual.VisualItem#getBounds()} are not
     *            accurate and the item's {@link prefuse.render.Renderer} is
     *            drawing outside of the reported bounds. In this case, there is
     *            usually a bug in the Renderer. One reported problem arises
     *            from Java itself, however, which inaccurately redraws images
     *            outside of their reported bounds. If you have a visulization
     *            with a number of images and are seeing rendering artifacts,
     ...
     * @param height
     *            the height
     * @return the new offscreen buffer */
        ...
        synchronized (m_vis) {
            synchronized (this) {
                ...
                if (m_damageRedraw) {
                    ...
                    m_rclip.setFrameFromDiagonal(m_clip.getMinX(), m_clip.getMinY(),
     ...
     * provide the maximum print quality. <b>This method may not be working
     * correctly, and will be repaired at a later date.</b>
     * 
     * @param g
     ...
     *             the noninvertible transform exception */
     ...
     * @param duration
     *            the duration of the animation, in milliseconds */
    public synchronized void animatePanAndZoomToAbs(final Point2D p, double scale,
            long duration) {
    ...
    protected class TransformActivity extends Activity {
        ...
        protected void run(long elapsedTime) {
            ...
            m_transform.setTransform(src[0] + f * (dst[0] - src[0]), src[1] + f
                    * (dst[1] - src[1]), src[2] + f * (dst[2] - src[2]), src[3] + f
                    * (dst[3] - src[3]), src[4] + f * (dst[4] - src[4]), src[5] + f
                    * (dst[5] - src[5]));
            try {
                m_itransform = m_transform.createInverse();
    ...
    public class InputEventCapturer implements MouseMotionListener, MouseWheelListener,
        ...
        /** Checks if is off component.
         * 
         * @param e
         *            the e
         * @return true, if is off component */
