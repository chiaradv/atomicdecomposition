...
public class NodeLinkTreeLayout extends TreeLayout {
    ...
    public NodeLinkTreeLayout(String group, int orientation, double dspace,
            double bspace, double tspace) {
        super(group);
    ...
    public void setOrientation(int orientation) {
        if (orientation < 0 || orientation >= Constants.ORIENTATION_COUNT
            ...
            throw new IllegalArgumentException("Unsupported orientation value: "
                    + orientation);
        }
        m_orientation = orientation;
    }
    ...
    private double spacing(NodeItem l, NodeItem r, boolean siblings) {
        ...
        return (siblings ? m_bspace : m_tspace)
                + 0.5
                * (w ? l.getBounds().getWidth() + r.getBounds().getWidth() : l
                        .getBounds().getHeight() + r.getBounds().getHeight());
    }
    ...
    private void executeShifts(NodeItem n) {
        ...
        for (NodeItem c = (NodeItem) n.getLastChild(); c != null; c = (NodeItem) c
                .getPreviousSibling()) {
    ...
    private void secondWalk(NodeItem n, NodeItem p, double m, int depth) {
        ...
        if (n.isExpanded()) {
            depth += 1;
            for (NodeItem c = (NodeItem) n.getFirstChild(); c != null; c = (NodeItem) c
                    .getNextSibling()) {
