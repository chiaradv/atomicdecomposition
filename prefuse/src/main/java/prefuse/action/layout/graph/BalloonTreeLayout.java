...
public class BalloonTreeLayout extends TreeLayout {
     ...
     * -bx; np.ry = -by; p = Math.PI; pr = 0; np.r = 0; childIter =
     * n.getChildren(); while ( childIter.hasNext() ) { NodeItem c =
     * (NodeItem)childIter.next(); ParamBlock cp = getParams(c); p += pr + cp.a
     * + fs; double x = cp.r*Math.cos(p)-bx; double y = cp.r*Math.sin(p)-by;
     * double d = Math.sqrt(x*x+y*y) + cp.r; np.r = Math.max(np.r,
     * (int)Math.round(d)); pr = cp.a; } if ( np.r == 0 ) np.r = m_minRadius +
     * 2*np.d; } // private void secondWalk2(NodeItem n, NodeItem r, double x,
     * double y, double l, double t) { ParamBlock np = getParams(n); double cost
     ...
     *            the t */
    private void
            secondWalk(NodeItem n, NodeItem r, double x, double y, double l, double t) {
        setX(n, r, x);
        setY(n, r, y);
        Params np = getParams(n);
        int numChildren = 0;
