...
public class Graph extends CompositeTupleSet {
    ...
    public Graph(Table nodes, boolean directed, String nodeKey, String sourceKey,
            String targetKey) {
    ...
    public Graph(Table nodes, Table edges, boolean directed) {
        this(nodes, edges, directed, DEFAULT_NODE_KEY, DEFAULT_SOURCE_KEY,
                DEFAULT_TARGET_KEY);
    }
     ...
     * @param targetKey
     *            data field used to denote the target node in an edge table */
    public Graph(Table nodes, Table edges, boolean directed, String sourceKey,
            String targetKey) {
     ...
     * @param targetKey
     *            data field used to denote the target node in an edge table */
    protected void init(Table nodes, Table edges, boolean directed, String nodeKey,
            String sourceKey, String targetKey) {
        // sanity check
     ...
     *         {@link CompositeTupleSet#getSet(String)} using {@link #NODES} as
     *         the parameter. */
     ...
     *         {@link CompositeTupleSet#getSet(String)} using {@link #EDGES} as
     *         the parameter. */
     ...
     * @return an iterator over all edge ids for edges incident on the given
     *         node {@link #INEDGES} (for in-linking edges), {@link #OUTEDGES}
     *         (for out-linking edges), or {@link #UNDIRECTED} (for all edges). */
    public IntIterator edgeRows(int node, int direction) {
        if (direction == OUTEDGES) {
            ...
            return new CompositeIntIterator(edgeRows(node, OUTEDGES), edgeRows(node,
                    INEDGES));
        } else {
            throw new IllegalArgumentException("Unrecognized edge type: " + direction
                    + ". Type should be one of Graph.OUTEDGES, "
    ...
    public Iterator tuples(Predicate filter) {
        if (filter == null) {
            ...
            return new CompositeIterator(m_edgeTuples.iterator(getEdgeTable()
                    .rows(filter)), m_nodeTuples.iterator(getNodeTable().rows(filter)));
        }
    }
     ...
     * the node table will be generated. Spanning trees are generated using an
     * unweighted breadth first search over the graph structure.
     * 
     ...
     * spanning tree for this Graph instance. Spanning trees are generated using
     * an unweighted breadth first search over the graph structure.
     * 
     * @param root
     ...
     * needed. This method is primarily useful for subclasses. For example,
     * calling this method on a Tree instance will revert the state to the
     * original rooted tree such that a sbusequent call to getSpanningTree()
     * will return the backing Tree itself.
     * 
     * @see #getSpanningTree()
     * @see #getSpanningTree(Node)
    ...
    protected void fireGraphEvent(Table t, int first, int last, int col, int type) {
        ...
        if (!m_listeners.isEmpty()) {
            ...
            for (int i = 0; i < lstnrs.length; ++i) {
                ((GraphListener) lstnrs[i]).graphChanged(this, table, first, last, col,
                        type);
            }
        }
    }
