...
public class GraphMLWriter extends AbstractGraphWriter {
    ...
    public void writeGraph(Graph graph, OutputStream os) throws DataIOException {
        ...
        xml.start(prefuse.data.io.GraphMLReader.Tokens.GRAPH,
                prefuse.data.io.GraphMLReader.Tokens.EDGEDEF,
                graph.isDirected() ? prefuse.data.io.GraphMLReader.Tokens.DIRECTED
                        : prefuse.data.io.GraphMLReader.Tokens.UNDIRECTED);
        // print the nodes
        xml.comment("nodes");
        ...
        while (nodes.hasNext()) {
            ...
            if (ns.getColumnCount() > 0) {
                xml.start(prefuse.data.io.GraphMLReader.Tokens.NODE,
                        prefuse.data.io.GraphMLReader.Tokens.ID,
                        String.valueOf(n.getRow()));
                for (int i = 0; i < ns.getColumnCount(); ++i) {
                    String field = ns.getColumnName(i);
                    xml.contentTag(prefuse.data.io.GraphMLReader.Tokens.DATA,
                            prefuse.data.io.GraphMLReader.Tokens.KEY, field,
                            n.getString(field));
                }
                xml.end();
            } else {
                xml.tag(prefuse.data.io.GraphMLReader.Tokens.NODE,
                        prefuse.data.io.GraphMLReader.Tokens.ID,
                        String.valueOf(n.getRow()));
            }
        }
        ...
        xml.println();
        // print the edges
        String[] attr = new String[] { prefuse.data.io.GraphMLReader.Tokens.ID,
                prefuse.data.io.GraphMLReader.Tokens.SOURCE,
                prefuse.data.io.GraphMLReader.Tokens.TARGET };
    ...
    private void printSchema(XMLWriter xml, String group, Schema s, String[] ignore) {
        String[] attr = new String[] { prefuse.data.io.GraphMLReader.Tokens.ID,
                prefuse.data.io.GraphMLReader.Tokens.FOR,
                prefuse.data.io.GraphMLReader.Tokens.ATTRNAME,
                prefuse.data.io.GraphMLReader.Tokens.ATTRTYPE };
        ...
        OUTER: for (int i = 0; i < s.getColumnCount(); ++i) {
            ...
            if (dflt == null) {
                ...
                xml.contentTag(prefuse.data.io.GraphMLReader.Tokens.DEFAULT,
                        dflt.toString());
                xml.end();
            }
        }
