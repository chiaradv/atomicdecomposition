...
public class DatabaseDataSource {
    ...
    private static final Logger s_logger = Logger.getLogger(DatabaseDataSource.class
            .getName());
    /** The m_conn. */
     ...
     *            the conn
     * @param handler
     *            the handler {@link ConnectionFactory} class should be used to
     *            create any number of DatabaseDataSource connections. */
     ...
     *             if an error occurs while executing the query or adding the
     *             query results in a prefuse Table. */
     ...
     *            the text SQL query to execute
     * @param keyField
     *            the field to treat as a primary key, ensuring that this field
     *            is indexed in the resulting table instance.
     * @return a Table of the query results
     * @throws DataIOException
     *             if an error occurs while executing the query or adding the
     *             query results in a prefuse Table. */
     ...
     * 
     * @param t
     *            the Table to store the results in. If this value is null, a
     ...
     *            used to determine if the row already exists in the table
     * @param lock
     *            an optional Object to use as a lock when performing data
     *            processing. This lock will be synchronized on whenever the
     *            Table is modified.
     * @return a Table of the query results
     * @throws DataIOException
     *             if an error occurs while executing the query or adding the
    ...
    public synchronized Table
            getData(Table t, String query, String keyField, Object lock)
                    throws DataIOException {
        ResultSet rs;
        try {
     ...
     *            the query to execute
     * @param keyField
     *            the primary key field, comparisons on this field are performed
     *            to recognize data records already present in the table. */
     ...
     * @param keyField
     *            the primary key field, comparisons on this field are performed
     *            to recognize data records already present in the table. A null
     *            value will result in no key checking.
     * @param lock
     *            an optional Object to use as a lock when performing data
     *            processing. This lock will be synchronized on whenever the
     *            Table is modified. A null value will result in no locking.
     * @param listener
     *            an optional listener that will provide notifications before
     *            the query has been issued and after the query has been
     *            processed. This is most useful for post-processing operations. */
    public void loadData(Table t, String query, String keyField, Object lock,
            DataSourceWorker.Listener listener) {
        DataSourceWorker.Entry e = new DataSourceWorker.Entry(this, t, query, keyField,
                lock, listener);
    ...
    private ResultSet executeQuery(String query) throws SQLException {
        ...
        s_logger.info("External query processing completed: " + time / 1000 + "." + time
                % 1000 + " seconds.");
        return rset;
    }
    ...
    protected Table process(Table t, ResultSet rset, String key, Object lock)
        ...
        // clock out
        long time = System.currentTimeMillis() - timein;
        s_logger.info("Internal query processing completed: " + count + " rows, " + time
                / 1000 + "." + time % 1000 + " seconds.");
        return t;
    }
     ...
     * @param rset
     *            the ResultSet, set to a particular row, which may or may not
