...
public class Table extends AbstractTupleSet implements ColumnListener {
     ...
     * columns will result in a runtime exception. If this Table subsequently
     * has columns added or removed, this will not be reflected in the returned
     * schema. Instead, this method will need to be called again to get a
     * current schema. Accordingly, it is not recommended that Schema instances
     * returned by this method be stored or reused across scopes unless that
     * exact schema snapshot is desired.
     * 
    ...
    public Schema getSchema() {
        if (m_schema == null) {
            ...
            for (int i = 0; i < getColumnCount(); ++i) {
                s.addColumn(getColumnName(i), getColumnType(i), getColumn(i)
                        .getDefaultValue());
            }
            s.lockSchema();
            m_schema = s;
    ...
    public void addColumn(String name, Class type, Object defaultValue) {
        Column col = ColumnFactory.getColumn(type, m_rows.getMaximumRow() + 1,
                defaultValue);
        addColumn(name, col);
    }
    ...
    protected void addColumn(String name, Column col) {
        ...
        if (idx >= 0 && idx < m_columns.size()) {
            throw new IllegalArgumentException("Table already has column with name \""
                    + name + "\"");
        }
        // add the column
        m_columns.add(col);
        ...
        // fire notification
        fireTableEvent(m_rows.getMinimumRow(), m_rows.getMaximumRow(), m_lastCol,
    ...
    protected Column removeColumn(int idx) {
        ...
        // fire notification
        fireTableEvent(m_rows.getMinimumRow(), m_rows.getMaximumRow(), idx,
     ...
     *            not be sorted.
     * @return a new table meeting the query specification {@link CascadedTable}
     *         class. */
     ...
     *         bounded range of a data field {@link prefuse.data.util.Index}
     *         class. */
