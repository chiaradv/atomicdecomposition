...
public class UILib {
     ...
     * @return a new Box instance with the given properties.
     * @see javax.swing.Box */
    public static Box getBox(Component[] c, boolean horiz, int margin1, int margin2,
            int spacing) {
    ...
    public static void addStrut(JComponent b, int layout, int size) {
        ...
        b.add(getAxis(b, layout) == BoxLayout.X_AXIS ? Box.createHorizontalStrut(size)
    ...
    public static void addGlue(JComponent b, int layout) {
        b.add(getAxis(b, layout) == BoxLayout.X_AXIS ? Box.createHorizontalGlue() : Box
                .createVerticalGlue());
    }
