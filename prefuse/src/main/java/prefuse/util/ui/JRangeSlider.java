...
public class JRangeSlider extends JComponent implements MouseListener,
     ...
     * rounding errors on toLocal and toScreen. With inclusion in prefuse, this
     * class has been further modified to use a bounded range model, support
     * keyboard commands and more extensize parameterization of
     * rendering/appearance options. Furthermore, a stub method has been
     * introduced to allow subclasses to perform custom rendering within the
     * slider through.
     */
        ...
        if (orientation == VERTICAL) {
            if (direction == LEFTRIGHT_TOPBOTTOM) {
                ...
                paintArrow(g2, (width - ARROW_WIDTH) / 2.0, min - ARROW_SZ
                        + (ARROW_SZ - ARROW_HEIGHT) / 2.0, ARROW_WIDTH, ARROW_HEIGHT,
                        true);
    ...
    protected void paintArrow(Graphics2D g2, double x, double y, int w, int h,
        ...
        if (orientation == VERTICAL) {
            ...
            if (topDown) {
                ...
                for (int i = 0; i < h / 2 + 1; i++) {
                    g2.drawLine(intX + i, intY + h / 2 - i, intX + i, intY + h - h / 2
                            + i - 1);
                }
            }
        }
     ...
     *            the height */
    protected void
            paint3DRectLighting(Graphics2D g2, int x, int y, int width, int height) {
    ...
    private void grow(int increment) {
        model.setRangeProperties(model.getValue() - increment, model.getExtent() + 2
                * increment, model.getMinimum(), model.getMaximum(), false);
    }
