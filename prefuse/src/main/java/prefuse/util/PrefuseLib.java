...
public class PrefuseLib {
    ...
    private static final double SIZE_SCALE_FACTOR = PrefuseConfig
            .getDouble("size.scale2D");
    /** Prefix for visualization-specific data fields, such as those used by
     * VisualItem; the default is a single underscore (_). */
    public static final String FIELD_PREFIX = PrefuseConfig
            .get("data.visual.fieldPrefix");
     ...
     * bottom-level group from the group name (e.g., graph.nodes to graph).
     * 
     * @param group
     *            the group name
     ...
     * bottom-level group from the group name (e.g., graph.nodes to nodes).
     * 
     * @param group
     *            the group name
