...
public class NBodyForce extends AbstractForce {
     ...
     */
    /** The pnames. */
    private static String[] pnames = new String[] { "GravitationalConstant", "Distance",
            "BarnesHutTheta" };
    ...
    public NBodyForce(float gravConstant, float minDistance, float theta) {
        ...
        minValues = new float[] { DEFAULT_MIN_GRAV_CONSTANT, DEFAULT_MIN_DISTANCE,
                DEFAULT_MIN_THETA };
        maxValues = new float[] { DEFAULT_MAX_GRAV_CONSTANT, DEFAULT_MAX_DISTANCE,
                DEFAULT_MAX_THETA };
     ...
     * @param y2
     *            the y2 */
    private void insert(ForceItem p, QuadTreeNode n, float x1, float y1, float x2,
            float y2) {
     ...
     * @param y2
     *            the y2 */
    private void insertHelper(ForceItem p, QuadTreeNode n, float x1, float y1, float x2,
            float y2) {
     ...
     * @param y2
     *            the y2 */
    private void forceHelper(ForceItem item, QuadTreeNode n, float x1, float y1,
            float x2, float y2) {
        ...
        if (!n.hasChildren && n.value != item || !same
                && (x2 - x1) / r < params[BARNES_HUT_THETA]) {
            if (minDist) {
                return;
            }
            ...
            for (int i = 0; i < n.children.length; i++) {
                if (n.children[i] != null) {
                    forceHelper(item, n.children[i], i == 1 || i == 3 ? splitx : x1,
                            i > 1 ? splity : y1, i == 1 || i == 3 ? x2 : splitx,
                            i > 1 ? y2 : splity);
                }
            }
            if (minDist) {
    ...
    public static final class QuadTreeNode {
        /** Instantiates a new quad tree node. */
        public QuadTreeNode() {
