 ...
 * broad picture. This class has been adapted from the corresponding class in
 * the COLT library for scientfic computing.
 * 
...
public class IntObjectHashMap extends AbstractHashMap implements Cloneable {
    ...
    public boolean put(int key, Object value) {
        ...
        if (distinct > highWaterMark) {
            int newCapacity = chooseGrowCapacity(distinct + 1, minLoadFactor,
                    maxLoadFactor);
            rehash(newCapacity);
    ...
    public boolean removeKey(int key) {
        ...
        if (distinct < lowWaterMark) {
            int newCapacity = chooseShrinkCapacity(distinct, minLoadFactor, maxLoadFactor);
            rehash(newCapacity);
        }
        return true;
    }
